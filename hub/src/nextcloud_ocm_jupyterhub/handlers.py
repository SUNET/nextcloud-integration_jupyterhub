...
class OCMLoginHandler(BaseHandler):
    ...
    async def post(self):
        ...
        try:
            ...
            return
        # Canonicalise so the session user matches the one the OCM service
        # spawned the server for (derived from the provisioned shareWith).
        username = f'ocm:{norm_ocm(claims["aud"])}'
